...
bigcnn_model.compile(loss="sparse_categorical_crossentropy",
                   optimizer=Adam(exp_decay_lr), metrics=["accuracy"])

# fit model with minibatches
history = bigcnn_model.fit(X_train, y_train,
						 epochs=epochs, batch_size=batch_size,
                         validation_data=(X_val, y_val),
                         callbacks=callbacks)

bigcnn_model.save('models/bigcnnmodel')

bigcnn_model.evaluate(X_test, y_test)